...
@Component({
  // moduleId: module.id,
  selector: 'assign-non-allocation',
  templateUrl: 'assign-non-allocation.component.html',
  styleUrls: ['assign-non-allocation.component.scss']
})
export class AssignNonAllocationComponent implements OnInit {

  displayedColumns: string[] = [
    'no', 'name', 'email', 'address', 'designName', 'contactNo', 'department',
    'status', 'joinDate', 'description', 'assignedCount', 'action'
  ];
  ...
  getUserData() {
    this.userservices.getAvailableEmployees().subscribe(
      (data: UserFormInterface[]) => {
        ...
        console.log('Available Employees:', this.dataSource.data);
      },
      (error: any) => {
  ...
  onTabChange(index: number) {
    ...
    if (index === 0) {
      this.routes.navigate(['app/assign-employee-to-client/assign/assignAllocation']);
    } else if (index === 1) {
      this.routes.navigate(['app/assign-employee-to-client/assign/assignNonAllocation']);
    }
  }
